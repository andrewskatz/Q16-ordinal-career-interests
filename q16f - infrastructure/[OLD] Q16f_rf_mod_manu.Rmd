---
title: "Question 16f - Resilient infrastructure"
author: "Katz et al."
date: "11/01/2022"
...
#imp_number <- 4
imp_number <- 5
climate_data <- climate_imp_long_full %>% filter(.imp == imp_number)
...
removal_vars <- c("School", "Litho", "Q29", "Q33_", 
                  "Q37_", "Q37", "Q38_", "Q39_", "Q40", "abe", "Q16abc", "lm", "dfg", "hijk",
...
factor_vars <- c("major", "Q32", "gender")
...
```{r}
extra_removal_vars <- c(".id", ".imp", "student_id",
...
#   ggtitle("Random Forest Variable Importance for Interest in Non-profit/NGO")
# 
# p
...
ord_rf_df <- train_df %>% 
  select(-one_of("Q7_disc_spec_env_sum", "Q7_disc_spec_env_sum",
                 "Q30", "Q31", "Q34", 
                 "Q35", "Q36a", "Q36b", 
                 "Q38", Q39_vars, "Q40")) %>% 
  drop_na(Q32, Q33, gender)
```
...
```{r}
var_imp_df %>% write_csv("Q16f_ord_rf_var_imp.csv")
```


```{r}
var_imp_df <- read_csv("Q16f_ord_rf_var_imp.csv")
```
...
p <- var_imp_df %>% 
  ...
  theme(axis.title.x = element_text(size = 10, color = "black"),
        ...
        axis.text.y  = element_text(size = 10, color = "black"),
        plot.title = element_text(size = 10, hjust = 0.5)) +
  ggtitle("Ordinal Forest Variable Importance for Interest in Working on Resilient Infrastructure")
...
file_name <- paste0("rf_ord_var_imp_Q16f_infra_", 
                    Sys.Date(),
                    "_imp",
                    imp_number,
                    ".png")
...
## Create logistic regression model with Q16f (working in entrepreneurship) and the following predictors:
...
# mod1_ord_Q16f_vs <- brm(formula = vs_formula,
#                      data = train_df,
...
mod1_ord_Q16f_vs <- brm_multiple(formula = vs_formula,
...
# file_name <- "mod1_ord_Q16f_vs.rds"
file_name <- "mod1_ord_Q16f_mi_vs.rds"

mod1_ord_Q16f_vs %>% write_rds(file = file_name)
...
mod1_ord_Q16f_vs <- read_rds("mod1_ord_Q16f_mi_vs.rds")
...
```{r}
summary(mod1_ord_Q16f_vs)
```

```{r}

loo(mod1_ord_Q16f_vs)
...
```{r}
# coef(mod1_ord_Q16k)
fixef(mod1_ord_Q16f_vs)

```

```{r}
get_variables(mod1_ord_Q16f_vs)
...
mod_param_vars <- tibble(var = get_variables(mod1_ord_Q16f_vs))
...
```{r}
stanplot(mod1_ord_Q16f_vs, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
mod1_ord_Q16f_vs %>%
  ...
  labs(title = "Intercept Estimates for Ordinal Regression for Interest in Working on Resilient Infrastructure",
...
```{r}
std_output <- c(".iteration", ".draw", ".chain")

param_plot <- mod1_ord_Q16f_vs %>%
  ...
  stat_halfeye(.width = c(.9, .5)) +
  labs(title = "Parameter Estimate Densities for Ordinal Regression for Interest in Working on Resilient Infrastructure",
       x = "Parameter Estimate",
       y = "Survey Item") +
  theme_light() +
...
ggsave(
  filename = paste0("rf_Q16f_vs_infra_halfeye_Qs_",
                    Sys.Date(),
                    ".png"),
  plot = param_plot_q,
  width = 7,
...
param_plot_major <- mod1_ord_Q16f_vs %>%
  spread_draws(`b_.*`, regex = TRUE) %>%
  pivot_longer(cols = starts_with("b_major"), names_to = "parameter", values_to = "parameter_estimate") %>% 
  ggplot(aes(y = factor(parameter), x = parameter_estimate)) +
  geom_vline(xintercept = 0, color = "black", linetype = "dashed") +
  stat_halfeye(.width = c(.9, .5)) +
  labs(title = "Parameter Estimate Densities for Ordinal Regression for Interest in Working on Resilient Infrastructure?",
...
```{r}
ggsave(
  filename = paste0("rf_Q16f_vs_infra_halfeye_majors_",
                    Sys.Date(),
                    ".png"),
  plot = param_plot_major,
  width = 7,
  height = 5,
  units = "in",
  dpi = 600
...
pp_plot_ecdf_vs <- pp_check(mod1_ord_Q16f_vs, type = "ecdf_overlay", nsamples = 100)+
  xlab("Response") + 
  ylab("Cumulative Probability") +
  ggtitle("Model Posterior Predictive Check",
          subtitle = "Interest in working on resilient infrastructure")
...
ggsave(paste0("pp_plot_ecdf_Q16f_vs_",
              Sys.Date(),
              ".png"), 
...
pp_plot_bars_vs <- pp_check(mod1_ord_Q16f_vs, type = "bars", nsamples = 500)+
...
ggsave(paste0("pp_plot_bars_Q16f_vs_",
              Sys.Date(),
              ".png"), 
