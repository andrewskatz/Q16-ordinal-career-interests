...
```{r}
var_imp_df %>% write_csv("Q16g_ord_rf_var_imp.csv")
```


```{r}
var_imp_df <- read_csv("Q16g_ord_rf_var_imp.csv")
```
...
p <- var_imp_df %>% 
  ...
  theme(axis.title.x = element_text(size = 10, color = "black"),
        ...
        axis.text.y  = element_text(size = 10, color = "black"),
        plot.title = element_text(size = 10, hjust = 0.5)) +
  ggtitle("Ordinal Forest Variable Importance for Interest in Working Alongside Local Gov")
...
file_name <- paste0("rf_ord_var_imp_Q16g_loc_gov_", 
                    Sys.Date(),
                    "_imp",
                    imp_number,
                    ".png")
...
## Create logistic regression model with Q16g (working in entrepreneurship) and the following predictors:
...
# mod1_ord_Q16g_vs <- brm(formula = vs_formula,
#                      data = train_df,
...
mod1_ord_Q16g_vs <- brm_multiple(formula = vs_formula,
...
# file_name <- "mod1_ord_Q16g_vs.rds"
file_name <- "mod1_ord_Q16g_mi_vs.rds"

mod1_ord_Q16g_vs %>% write_rds(file = file_name)
...
mod1_ord_Q16g_vs <- read_rds("mod1_ord_Q16g_mi_vs.rds")
...
```{r}
summary(mod1_ord_Q16g_vs)
```

```{r}

loo(mod1_ord_Q16g_vs)
...
```{r}
# coef(mod1_ord_Q16k)
fixef(mod1_ord_Q16g_vs)

```

```{r}
get_variables(mod1_ord_Q16g_vs)
...
mod_param_vars <- tibble(var = get_variables(mod1_ord_Q16g_vs))
...
```{r}
stanplot(mod1_ord_Q16g_vs, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
mod1_ord_Q16g_vs %>%
  ...
  labs(title = "Intercept Estimates for Ordinal Regression for Interest in Working Alongside Local Gov",
...
param_plot <- mod1_ord_Q16g_vs %>%
  ...
  stat_halfeye(.width = c(.9, .5)) +
  labs(title = "Parameter Estimate Densities for Ordinal Regression for Interest in Working Alongside Local Gov",
       x = "Parameter Estimate",
       y = "Survey Item") +
  theme_light() +
...
ggsave(
  filename = paste0("rf_Q16g_vs_loc_gov_halfeye_Qs_",
                    Sys.Date(),
                    ".png"),
  plot = param_plot_q,
  width = 7,
...
```{r}
# 
# param_plot_major <- mod1_ord_Q16g_vs %>%
#   spread_draws(`b_.*`, regex = TRUE) %>%
...
```{r}
# ggsave(
#   filename = paste0("rf_Q16g_vs_loc_gov_halfeye_majors_",
#                     Sys.Date(),
#                     ".png"),
...
pp_plot_ecdf_vs <- pp_check(mod1_ord_Q16g_vs, type = "ecdf_overlay", nsamples = 100)+
  xlab("Response") + 
  ylab("Cumulative Probability") +
  ggtitle("Model Posterior Predictive Check",
          subtitle = "Interest in working Alongside Local Gov")
...
ggsave(paste0("pp_plot_ecdf_Q16g_vs_",
              Sys.Date(),
              ".png"), 
...
pp_plot_bars_vs <- pp_check(mod1_ord_Q16g_vs, type = "bars", nsamples = 500)+
...
ggsave(paste0("pp_plot_bars_Q16g_vs_",
              Sys.Date(),
              ".png"), 
