---
title: "Question 16j - Fight Bioattacks and Pandemics - bioattacks"
author: "Katz et al."
date: "11/08/2022"
...
```{r notebook vars}
analysis_item <- "Q16j"
save_topic <- "bioattacks"
topic <- "Fight Bioattacks and Pandemics"
```
...
# mod1_ord_Q16j_vs <- brm(formula = vs_formula,
#                      data = train_df,
...
mod1_ord_Q16j_vs <- brm_multiple(formula = vs_formula,
...
# file_name <- "mod1_ord_Q16j_vs.rds"
file_name <- "mod1_ord_Q16j_mi_vs.rds"

mod1_ord_Q16j_vs %>% write_rds(file = file_name)
...
mod1_ord_Q16j_vs <- read_rds("mod1_ord_Q16j_mi_vs.rds")
...
```{r}
summary(mod1_ord_Q16j_vs)
```

```{r}

loo(mod1_ord_Q16j_vs)
...
```{r}
# coef(mod1_ord_Q16j)
fixef(mod1_ord_Q16j_vs)

```

```{r}
get_variables(mod1_ord_Q16j_vs)
...
mod_param_vars <- tibble(var = get_variables(mod1_ord_Q16j_vs))
...
```{r}
stanplot(mod1_ord_Q16j_vs, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
file_name <- paste(analysis_item,
                   ...
                   sep = "_")
# ggsave(
#   filename = "Q16j_bioattacks_halfeye_Qs_20220930.png",
#   plot = param_plot_q,
#   width = 7,
#   height = 5,
#   units = "in",
...
pp_plot_ecdf_vs <- pp_check(mod1_ord_Q16j_vs, type = "ecdf_overlay", nsamples = 100)+
  xlab("Response") + 
...
pp_plot_bars_vs <- pp_check(mod1_ord_Q16j_vs, type = "bars", nsamples = 500)+
...
p <- var_imp_df %>%
  ...
  mutate(variable = case_when(variable == "Q4p" ~ "Career sat.: Volunteering",
                              ...
                              variable == "Q16j" ~ "Career: Fight Bioattacks and Pandemics",
                              ...
                              variable == "Q16j" ~ "Career: Work alongside gov.",
                              variable == "Q18j" ~ "Think of myself as part of nature",
                              variable == "poli_aff" ~ "Political affiliation",
                              variable == "Q16j" ~ "Career: Work on Fight Bioattacks and Pandemics",
