---
title: "Question 16l - Clean Water Tech - watertech"
author: "Katz et al."
date: "11/08/2022"
...
```{r notebook vars}
analysis_item <- "Q16l"
save_topic <- "watertech"
topic <- "Clean Water Tech"
```
...
# mod1_ord_Q16l_vs <- brm(formula = vs_formula,
#                      data = train_df,
...
mod1_ord_Q16l_vs <- brm_multiple(formula = vs_formula,
...
# file_name <- "mod1_ord_Q16l_vs.rds"
file_name <- "mod1_ord_Q16l_mi_vs.rds"

mod1_ord_Q16l_vs %>% write_rds(file = file_name)
...
mod1_ord_Q16l_vs <- read_rds("mod1_ord_Q16l_mi_vs.rds")
...
```{r}
summary(mod1_ord_Q16l_vs)
```

```{r}

loo(mod1_ord_Q16l_vs)
...
```{r}
# coef(mod1_ord_Q16l)
fixef(mod1_ord_Q16l_vs)

```

```{r}
get_variables(mod1_ord_Q16l_vs)
...
mod_param_vars <- tibble(var = get_variables(mod1_ord_Q16l_vs))
...
```{r}
stanplot(mod1_ord_Q16l_vs, pars = c("^r_", "^b_", "^sd_")) +
  theme_light() +
...
file_name <- paste(analysis_item,
                   ...
                   sep = "_")
# ggsave(
#   filename = "Q16l_watertech_halfeye_Qs_20220930.png",
#   plot = param_plot_q,
#   width = 7,
#   height = 5,
#   units = "in",
...
pp_plot_ecdf_vs <- pp_check(mod1_ord_Q16l_vs, type = "ecdf_overlay", nsamples = 100)+
  xlab("Response") + 
...
pp_plot_bars_vs <- pp_check(mod1_ord_Q16l_vs, type = "bars", nsamples = 500)+
...
p <- var_imp_df %>%
  ...
  mutate(variable = case_when(variable == "Q4p" ~ "Career sat.: Volunteering",
                              ...
                              variable == "Q16l" ~ "Career: Clean Water Tech",
